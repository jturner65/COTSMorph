...
public class SpiralTransform extends baseTransform {
     ...
     */
    @Override
    public final float drawRightSideBarMenuDescr(IGraphicsAppInterface ri, float yOff, float sideBarYDisp, String coordName) {
        ...
        ri.pushMatState();
        showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_White, 255), 3.5f, coordName + " :");
        for(int j=0;j<rtMenuDispType.length;++j) {            
            showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_White, 255), 3.0f, rtMenuDispType[j]+" :");
            showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_LightGreen, 255), 5.7f, dispVals[j]);
        }
...
//        ri.pushMatState();        
        if(isDegenerate) {
            showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_White, 255), 3.5f, "No F");
        } else {
            
            showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_White, 255), 3.5f, "F : ");
            showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_LightCyan, 255), 3.0f, "("+F.toStrCSV(Base_PolyMap.strPointDispFrmt8)+")");
        }
        ri.popMatState();
    ...
    public final float drawFixedPoint(IGraphicsAppInterface ri, float yOff, float sideBarYDisp){
        ri.translate(10.0f, 0.0f, 0.0f);
        if(isDegenerate) {
            showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_White, 255), 3.5f, "Theta == 0 caused degenerate spiral.");
        } else {
            ri.pushMatState();        
                showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_White, 255), 5.5f, "Fixed Point : ");
                showOffsetText_RightSideMenu(ri, ri.getClr(IGraphicsAppInterface.gui_LightCyan, 255), 3.0f, "("+F.toStrCSV(Base_PolyMap.strPointDispFrmt8)+")");
            ri.popMatState();
        }
        yOff += sideBarYDisp;
